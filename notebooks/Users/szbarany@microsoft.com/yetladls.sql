...
-- MAGIC %sql
-- MAGIC create view adls_transaction_aggregate AS
-- MAGIC SELECT 
...
resultDF.coalesce(1).write.format("com.databricks.spark.csv").option("header", "true").save("/mnt/adls/adls_transaction_aggregate.csv")
...
-- MAGIC %sql
-- MAGIC CREATE  VIEW adls_articles_tr_aggr AS 
-- MAGIC SELECT 
...
resultDF.coalesce(1).write.format("com.databricks.spark.csv").option("header", "true").save("adls_articles_tr_aggr")